#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: BP_Nvzhu

#include "Basic.hpp"

#include "TsBaseCharacter_classes.hpp"


namespace SDK
{

// BlueprintGeneratedClass BP_Nvzhu.BP_Nvzhu_C
// 0x0020 (0x0780 - 0x0760)
class ABP_Nvzhu_C final : public ATsBaseCharacter_C
{
public:
	class UKuroAdjustableCapsuleComponent*        Bip001LThighTwist;                                 // 0x0758(0x0008)(BlueprintVisible, ZeroConstructor, InstancedReference, IsPlainOldData, NonTransactional, NoDestructor, HasGetValueTypeHash)
	class UKuroAdjustableCapsuleComponent*        Bip001RThighTwist;                                 // 0x0760(0x0008)(BlueprintVisible, ZeroConstructor, InstancedReference, IsPlainOldData, NonTransactional, NoDestructor, HasGetValueTypeHash)
	class UKuroAdjustableCapsuleComponent*        Bip001Spine2;                                      // 0x0768(0x0008)(BlueprintVisible, ZeroConstructor, InstancedReference, IsPlainOldData, NonTransactional, NoDestructor, HasGetValueTypeHash)
	class USkeletalMeshComponent*                 WeaponCase1;                                       // 0x0770(0x0008)(BlueprintVisible, ZeroConstructor, InstancedReference, IsPlainOldData, NonTransactional, NoDestructor, HasGetValueTypeHash)
	class USkeletalMeshComponent*                 WeaponCase0;                                       // 0x0778(0x0008)(BlueprintVisible, ZeroConstructor, InstancedReference, IsPlainOldData, NonTransactional, NoDestructor, HasGetValueTypeHash)

public:
	void FightCommand(bool isInAir);

public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"BP_Nvzhu_C">();
	}
	static class ABP_Nvzhu_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<ABP_Nvzhu_C>();
	}
};
static_assert(alignof(ABP_Nvzhu_C) == 0x000010, "Wrong alignment on ABP_Nvzhu_C");
static_assert(sizeof(ABP_Nvzhu_C) == 0x000780, "Wrong size on ABP_Nvzhu_C");
static_assert(offsetof(ABP_Nvzhu_C, Bip001LThighTwist) == 0x000758, "Member 'ABP_Nvzhu_C::Bip001LThighTwist' has a wrong offset!");
static_assert(offsetof(ABP_Nvzhu_C, Bip001RThighTwist) == 0x000760, "Member 'ABP_Nvzhu_C::Bip001RThighTwist' has a wrong offset!");
static_assert(offsetof(ABP_Nvzhu_C, Bip001Spine2) == 0x000768, "Member 'ABP_Nvzhu_C::Bip001Spine2' has a wrong offset!");
static_assert(offsetof(ABP_Nvzhu_C, WeaponCase1) == 0x000770, "Member 'ABP_Nvzhu_C::WeaponCase1' has a wrong offset!");
static_assert(offsetof(ABP_Nvzhu_C, WeaponCase0) == 0x000778, "Member 'ABP_Nvzhu_C::WeaponCase0' has a wrong offset!");

}

